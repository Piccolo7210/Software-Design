...
io.on('connection', (socket) => {
  socket.on('join-room', ({ roomId, playerName }) => {
    ...
    if (rooms[roomId].players.length < 2) {
      ...
      if (rooms[roomId].players.length === 2) {
        ...
        io.to(roomId).emit('player-joined', { playerName: rooms[roomId].players[1].name }); // Notify all players
      } else {
  ...
  socket.on('make-move', ({ roomId, col, player }) => {
    ...
    if (checkWin(board, player)) {
      io.to(roomId).emit('game-over', { board: board, winner: player, winnerName: room.players.find(p => p.playerNumber === player)?.name });
    } else if (isBoardFull(board)) {
      io.to(roomId).emit('game-over', { board: board, isDraw: true });
    }
  });
